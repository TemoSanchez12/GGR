
namespace GGR.Server.Errors;

public class RewardClaimErrorMessage
{
    public static string UserNotFound = "Usuario no encontrado";
    public static string UserNotVerified = "El usuario no esta verificado";
    public static string RewardNotFound = "No se ha encontrado la recompensa";
    public static string NotEnoughPoints = "Puntos no suficientes";
    public static string ErrorSavingData = "Ha ocurrido un error al guardar la información";
    public static string RewardClaimNotFound = "No se ha encontrado el reclamo de la recompensa";
    public static string NoAllowToAssignStatus = "No tiene permiso para cambiar el estado";
}
